	...
	_createClass(PixiStats, [
		{
			...
			value: function init() {
				...
				this.canvas.style.cssText = 'width:'
					.concat(this.config.baseCanvasWidth * this.main.config.scale, 'px;height:')
					.concat(
		...
		},
		{
			...
			value: function collectStats() {
				...
				var formatToSize =
					((_formatToSize = {}),
					_defineProperty(_formatToSize, this.pixi.FORMATS.RGB, 3),
					_defineProperty(_formatToSize, this.pixi.FORMATS.RGBA, 4),
					_defineProperty(_formatToSize, this.pixi.FORMATS.DEPTH_COMPONENT, 3),
					_defineProperty(_formatToSize, this.pixi.FORMATS.DEPTH_STENCIL, 4),
					_defineProperty(_formatToSize, this.pixi.FORMATS.ALPHA, 1),
					_defineProperty(_formatToSize, this.pixi.FORMATS.LUMINANCE, 1),
					_defineProperty(_formatToSize, this.pixi.FORMATS.LUMINANCE_ALPHA, 2),
					_formatToSize);
				var textures = this.app.renderer.texture.managedTextures;
				var buffers = this.app.renderer.buffer
					? this.app.renderer.buffer.managedBuffers
					: this.app.renderer.geometry.managedBuffers;
		...
		},
		{
			...
			value: function drawCounts(counts) {
				...
				ctx.fillText(
					''.concat(this.formatNum(textures)),
					startX,
					avgMinMaxOffsetY + padding
				); // buffers

				ctx.fillStyle = mainConfig.COLOR_FPS_BAR;
				ctx.fillText(
					''.concat(this.formatNum(buffers)),
					startX + avgMinMaxOffsetX * 1.51,
					avgMinMaxOffsetY + padding
				); // render texture

				ctx.fillStyle = mainConfig.COLOR_FPS_BAR;
				ctx.fillText(
					''.concat(this.formatNum(renderTextures)),
					startX + avgMinMaxOffsetX * 3.2,
					avgMinMaxOffsetY + padding
				); //draw calls
		...
		},
		{
			...
			value: function drawMem(mem) {
				...
				var imageData = ctx.getImageData(
					1,
					0,
					ctx.canvas.width - this.memGraph.barWidth,
					ctx.canvas.height
				);
				...
				ctx.fillText(
					'mem-buf',
					avgMinMaxOffsetX * 2.2 - padding * 2,
					avgMinMaxOffsetY + padding
				);
				this.graphYOffset = 0;
			}
		},
		{
			...
			value: function drawMemGraph(value, color, prevValue) {
				...
				var mainConfig = this.main.config;
				var ctx = this.ctx;
				if (prevValue && value + prevValue > config.maxMemorySize)
					value = Math.max(0, config.maxMemorySize - prevValue);
				var yOffset = 0;
				if (this.graphYOffset) yOffset += this.graphYOffset;
				var x =
					mainConfig.maximumHistory * this.memGraph.barWidth - this.memGraph.barWidth;
