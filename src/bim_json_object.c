...
const bim_json_object_t *bim_json_copy(const bim_json_object_t *bim_object) {
    ...
    for (size_t i = 0; i < levels_count; ++i) {
        ...
        bim_json_element_t *elements = (bim_json_element_t *) malloc(
                sizeof(bim_json_element_t) * level->numofelements);
        if (!elements) {
            LOG_ERROR(
                    "Не удалось выделить память для структуры `bim_json_element_t` при копировании");
            free(bim);
            free(levels);
            return NULL;
        }
